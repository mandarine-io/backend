...
func setupDatabase(c *Container) {
	...
	// Setup GORM plugins
	err := gormPlugin.UseCachePlugin(c.DB, db_cacher.NewDbCacher(c.Cache.Manager))
	if err != nil {
		log.Warn().Err(err).Msg("failed to use cache plugin")
	}
	...
	if err != nil {
		log.Warn().Err(err).Msg("failed to migrate database")
	}
}
